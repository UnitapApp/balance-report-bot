...
def report():
    ...
    for chain in data:
        ...
        # contract balance
        t += f"""C: {str(chain['contractBalance'] / 10**int(chain["decimals"]))[:5]} {chain['symbol']} {tick if chain["hasEnoughFunds"] is True else cross}"""

        # 4 spaces
        t += " "*4

        # wallet balance
        t += f"""W: {str(chain['walletBalance'] / 10**int(chain["decimals"]))[:5]} {chain['symbol']} {tick if chain["hasEnoughFees"] is True else cross}"""
